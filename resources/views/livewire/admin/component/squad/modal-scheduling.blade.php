<div class="p-4">
    <div class="flex justify-between">
        <h5 class="text-lg font-weight-bold">Tambah Jadwal Squad</h5>
    ...
    <form class="w-full max-w-full" wire:submit.prevent="submit" autocomplete="off">
        <div class="flex flex-wrap mb-6">
            <x-input-label for="name" value="Jadwal" />
            <select
                ...
                name="coordinator_id" wire:model.lazy="schedule_id">
                <option value="">Pilih Jadwal</option>
                @foreach ($shift_schedules as $c)
                    <option value="{{ $c->id }}">Hari {{ $c->day }}, shift {{ $c->shift }}</option>
                @endforeach
            </select>
            @error('schedule_id')
